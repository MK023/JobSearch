# Derived from backend/src/integrations/anthropic_client.py — keep core logic in sync
"""Local Anthropic API client for MCP server.

Provides analyze_job() for local batch processing without Fly.io dependency.
No Redis cache — runs directly on the Mac.
"""
...
def analyze_job(
    ...
    Simplified version without Redis cache — runs locally on MCP server.
    """
