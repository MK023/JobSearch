...
class JobOffer(WorldwildBase):
    ...
    url: Mapped[str | None] = mapped_column(String(1000), default="")
    ...
    salary_currency: Mapped[str | None] = mapped_column(String(8), default="")
    contract_type: Mapped[str | None] = mapped_column(String(32), default="")  # permanent | contract | …
    contract_time: Mapped[str | None] = mapped_column(String(32), default="")  # full_time | part_time | …
    ...
    pre_filter_passed: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, index=True)
    pre_filter_reason: Mapped[str | None] = mapped_column(String(255), default="")
    ...
    cv_match_score: Mapped[int | None] = mapped_column(nullable=True, index=True)
...
class Decision(WorldwildBase):
    ...
    decision: Mapped[str] = mapped_column(String(16), nullable=False, default=DECISION_PENDING, index=True)
    ...
    promoted_to_neon_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    ...
    promotion_score: Mapped[int | None] = mapped_column(nullable=True)
    promotion_started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    promotion_error: Mapped[str] = mapped_column(String(500), nullable=False, default="", server_default="")
...
class AdapterRun(WorldwildBase):
    ...
    source: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    run_type: Mapped[str] = mapped_column(String(16), nullable=False, default=RUN_TYPE_MANUAL)
    ...
    offers_fetched: Mapped[int] = mapped_column(default=0, nullable=False)
    offers_new: Mapped[int] = mapped_column(default=0, nullable=False)
    offers_pre_filtered_out: Mapped[int] = mapped_column(default=0, nullable=False)
