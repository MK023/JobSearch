...
def _serialize_batch_result_row(item: BatchItem, analysis: Any) -> dict[str, Any]:
    """Build the per-row payload returned by ``/batch/results`` for one completed item."""
    ...
    return {
        ...
        "is_duplicate": item.status == BatchItemStatus.SKIPPED,
...
def batch_results_route(request: Request, user: CurrentUser, db: DbSession) -> JSONResponse:
    ...
    for item in items:
        ...
        if not analysis:
            continue
        results.append(_serialize_batch_result_row(item, analysis))
